...
export class UserService {
  private _userRegisterUrl = `${environment.nodejs_api_host}${environment.nodejs_api_route.user.register}`;
  ...
  UserRegister(user: UserDto): Promise<HttpResponse<MessageModel>> {
    const res = this.http.post<MessageModel>(this._userRegisterUrl, user, { observe: "response" }).toPromise();
    return res;
  }
